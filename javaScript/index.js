...
const filtrarProductos = (
  ...
  let productosFiltrados = listaDeProductos.filter(
    (producto) =>
      categoriaMinusculas === "all" ||
      producto.categoria.toLowerCase() === categoriaMinusculas
  );
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  precioItems.forEach((item) => {
    item.addEventListener("click", function (event) {
      ...
      if (rangoPrecioString === "all") {
        ...
        const rangoPrecioArray = rangoPrecioString
          .split(" ")
          .map((str) => parseFloat(str.replace("$", "")));
        rangoPrecioActual =
          rangoPrecioArray.length === 2 ? rangoPrecioArray : "all";
      }
  ...
  const buscarPorNombre = function (nombre, listaDeProductos) {
    ...
    const productosFiltrados = listaDeProductos.filter((producto) =>
      producto.nombre.toLowerCase().includes(nombreMinusculas)
    );
  ...
  console.log(
    "\nOrden descendente:",
    ordenarPorPrecio([...listaDeProductos], false)
  );
  ...
  const figureElements = document.querySelectorAll(
    ".products figure, .productsOne figure, .products2 figure, .products3 figure"
  );

  // Asigna el contenido del array a los elementos figure en orden
  for (
    let i = 0;
    i < listaDeProductos.length && i < figureElements.length;
    i++
  ) {
